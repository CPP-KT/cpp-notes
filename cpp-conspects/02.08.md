...
```
_TODO почему-то 2 и 3 компилируются только в представленном выше порядке регистров_
