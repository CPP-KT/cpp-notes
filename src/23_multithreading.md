...
## Ещё немного про блокирующие операции
...
-  `abi::__force_unwind` нельзя ловить не пробрасывать, поэтому, если в программе есть `catch(...)`, который не пробрасывает исключение, тогда `pthread_cancel` тоже использовать нельзя.