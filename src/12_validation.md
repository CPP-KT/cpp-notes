...
1. `std::abort`\
`+` **хорошо взаимодействует с отладчиками** - отладчики умеют показывать место, где упала программа. Например, linux умеет создать дамп своей памяти перед `std::abort`, и в отладчике можно посмотреть, что было перед падением программы.\
...
В С++ `assert` реализован первым способом.
