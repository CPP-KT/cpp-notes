...
int main() {
    ...
    NonEmptyContainer<int, 0> a3; // ошибка компиляции
}
```
...
Рассмотрим, к примеру, две функции класса `std::vector` &mdash; `operator[](size_t pos)` и `at(size_t pos)`. Обе функции возвращают `pos`-тый элемент вектора. При этом, у квадратных скобок будет узкий контракт &mdash; корректность индекса не проверяется, и вызов `a[a.size() + 10]` приведёт к UB. Если же `at(size_t pos)` подать некорректный индекс, то она бросит исключение. Для любого входа поведение определено, значит её контракт широкий.
