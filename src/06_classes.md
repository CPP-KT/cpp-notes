...
Выше рассмотрели: 
1) Способ выделить память и вызвать на ней конструктор (`new`)
2) Способ только выделить память, не вызывая конструктор (`operator new`)

Но есть также ещё один способ &mdash; **вызвать конструктор на уже выделенной памяти**.
