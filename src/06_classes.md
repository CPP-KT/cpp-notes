...
Бывают случаи, когда мы хотим что-то добавить и максимально спрятать от пользовательского кода. И это нам позволит избежать перекомпиляции пользовательского кода. Или мы хотим выпустить новую версию библиотеки, так чтобы программы не компилировались снова, а их можно было просто "подложить". 
Как такое реализуют? Создают класс, внутри которого один указатель на какую-то структуру, которую снаружи никак не видно и реализуют все что надо в ней. Отсюда, собственно, и название идиомы Pimple - "Pointer to implementation". 
