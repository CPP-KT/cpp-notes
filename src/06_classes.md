...
## Аллокация памяти и конструирование объектов.
Итак, мы обсудили, когда вызываются конструкторы и деструкторы. А если вы хотите вызывать их в произвольный момент, конструируя объект тогда, когда захочет пользователь?

Как это работает? В C есть `malloc` и `free`, они выделяют память на куче в тот момент, когда вы её попросите (опять же, в рандомный момент). Выделение на куче, кстати, немного дороже, чем на стеке. Ниже рассмотрим несколько способов, как выделить память в C++.

1) `malloc` и `free`

``` c++
void* p1 = std::malloc(42); // Выделяет 42 байта.
std::free(p1);              // Освобождает память, выделенную ранее.
std::free(p1);              // Освобождать то, что уже освободили нельзя.

std::free(nullptr)          // Вот так можно, ничего не произойдёт.
```
Двойной `free` вызывает UB или даже [уязвимость](https://awakened1712.github.io/hacking/hacking-whatsapp-gif-rce/). Если же malloc вернул `nullptr`, то это означает, что память не выделилась.

2) `operator new` и `operator delete`

``` c++
void* p2 = operator new(42); // Выделяет 42 байта. 
operator delete(p2);         // Освобождает память, выделенную ранее.
```
`operator new` почти тоже самое, что и `malloc`, однако, если память не выделилась, то бросает исключение.

3) `new` и `delete`

``` c++
int* p3 = new int(42); // operator new + constructor
delete(p3);            // destructor + operator delete
```
В отличие от предыдущих вариантов, `new` и `delete` вызывают конструктор и деструктор объекта соответственно. По сути `new` просто является комбинацией из `operator new` (про который выше) и [вызова конструктора на выделенной памяти](https://en.cppreference.com/w/cpp/language/new#Placement_new) (про который ниже). Аналогично `delete` вызывает деструктор и `operator delete`.
...
delete[] p4;
```
Это `new` и `delete`, но операция массовая. На всём массиве вызываются конструкторы по умолчанию.

При этом ни в коем случае **нельзя освобождать объект другим способом, нежели он был выделен**. Это в любом случае **undefined behaviour**:

``` c++
void* p5 = std::malloc(42);
operator delete(p5); // Освободили другим способом => получили UB

mytype* p6 = new mytype[10];
delete p6; // Тоже освободили другим способом => опять получили UB
```

### Placement new

Выше рассмотрели: 1) способ выделить память и вызвать на ней конструктор (`new`), 2) способ только выделить память, не вызывая конструктор (`operator new`), но есть также и 3) способ вызвать конструктор на уже выделенной памяти.

Для этого служит `placement new` (размещающий `new`) &mdash; специальная форма вызова `new`, позволяющая дополнительным аргументом передать адрес, на котором будет вызван конструктор.

``` C++
mytype* buffer = static_cast<mytype*>(operator new(...));

for (size_t i = 0; i != 5; ++i)
	new(buffer + i) mytype(1., 2.); // можно std::construct_at(...);

for (size_t i = 5; i != 0; --i)
	buffer[i - 1].~mytype();        // можно std::destroy_at(...);
