...
```
Это называется *swap-trick*, и его мы ещё [обсудим в контексте исключений](./09_exceptions.md#swap-trick). Кстати, оператор присваивания также генерируется компилятором. Также покомпонентный. То, что компилятор генерирует сам, называется *специальными функциями-членами класса*. Это:
