...
Как это работает? В C есть `malloc` и `free`, они выделяют память на куче в тот момент, когда вы её попросите (опять же, в произвольный момент). Выделение на куче, кстати, немного дороже, чем на стеке. Ниже рассмотрим несколько способов, как выделить память в C++.
