...
``` c++
void* p1 = malloc(42); // Выделяет 42 байта. 
free(p1);              // Чистит память, выделенную ранее.
```
Если malloc вернул `nullptr`, то это означает, что память не выделилась.

2) `operator new` и `operator delete`

``` c++
  void* p2 = operator new(42); // Выделяет 42 байта. 
  operator delete(p2);         // Чистит память, выделенную ранее.
```
...
```
`new` аллоцирует память под переменную, а после на ней инициализирует переменную, вызовом конструктора.

`delete` же вызывает деструктор, а после возвращает память назад.
...
`placement new` - специальная форма вызова `new`, когда мы по какому то адресу вызываем конструктор.
