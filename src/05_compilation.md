...
И ещё есть *макросы*: определить макрос (`#define`) и разопределить макрос (`#undef`). Самый простой макрос — *object-like*: `#define MACRO something`, чуть более сложный — *function-like*: `#define MACRO(x) some x thing`. Макросы — это просто замена, которую в коде после них нужно провести.
...
```
И тут компилятор видит не то, что вы назвали переменную как нельзя.

Более страшные вещи получаются, когда пишется что-то такое:
```c++
#define mul(x, y) x * y
