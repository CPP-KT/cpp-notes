...
```
Что делает ключ *-C*, [оставим на потом](https://github.com/lejabque/cpp-notes/blob/master/src/05_compilation.md#Декорирование%20имён.%20%60extern%20"C"%60.). На то что тут находится `puts` вместо `printf`, тоже обращать внимание не надо, это просто такая оптимизация компилятора — когда можно заменить `printf` на `puts`, заменяем.\
...
Одно из применений `#ifndef` — это include guard, которые уже обсуждались [ранее](https://github.com/lejabque/cpp-notes/blob/master/src/05_compilation.md#headers).
