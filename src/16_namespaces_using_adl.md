...
int main() {
	...
	n1::foo b; // Qualified lookup ищет сначала в самом пространстве, потом в inline-namespace'ах, и уже в конце идёт по using-директивам.
...
Поэтому есть правило, которое называется [*argument-dependent lookup*](https://en.cppreference.com/w/cpp/language/adl). Когда мы вызываем функцию, она ищется не как описано выше, а учитывает типы параметров. Точнее, смотрит в то пространство имён, где написаны аргументы оператора. **Для каждого аргумента производится поиск в его пространстве имён (и пространствах имён всех его баз)**. Причём только в пространстве имён, не выше.
