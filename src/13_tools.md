...
### SAL-аннотации.
Хорошо, статические анализаторы GCC и Clang межпроцедурные и даже могут смотреть сквозь единицы трансляции. В Visual Studio, увы, не так, поэтому вот такой код:
```c++
void f(int* val) {
    printf("%d", *val);
}
