...
Возникает вопрос: а зачем писать `tuple<T...>`, почему не `tuple<T>`? Ну, потому что тогда непонятно, что должно произойти тут: `tuple<tuple<T>>`. Это `tuple<tuple<T1, T2, ...>>` или `tuple<tuple<T1>, tuple<T2>, ...>`? Поэтому вы явно пишете многоточие там, где хотите, чтобы все аргументы перечислились через запятую.
