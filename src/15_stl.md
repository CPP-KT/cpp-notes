...
**Input** и **output** итераторы позволяют пройти по диапазону только один раз. При этом **input** итераторы позволяют читать данные, а **output** итераторы — записывать.
...
При инкременте **input** или **output** итератора все его копии инвалидируются.

При **инвалидации** итераторы перестают указывать туда, куда должны были указывать; к примеру, в `std::vector`'е после удаления некого элемента происходит инвалидация итераторов, указывающих на него или на любой элемент после него, а после добавления элемента в случае, когда `size == capacity`, инвалидируются все итераторы).

**Forward** итераторы позволяют пройтись по диапазону несколько раз. **Forward** итератор поддерживает все те же операции, что и **input** итератор, и, если он изменяемый, то он также поддерживает операции, поддерживаемые **output** итератором. **forward** итераторы, в отличие от **input** и **output** итераторов, поддерживают постинкремент — `it++`.
...
**Random access** итераторы поддерживают такие контейнеры, как `std::vector`, `std::array`. Указатели также поддерживают интерфейс **random access** итераторов. **Random access** итераторы требуются для функций, где необходимо уметь прыгать через n элементов: `сортировки, k-я порядковая статистика, бинарный поиск` и т.д.
