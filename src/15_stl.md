...
**Forward** итераторы позволяют пройтись по диапазону несколько раз. **Forward** итератор поддерживает все те же операции, что и **input** итератор, и, если он изменяемый, то он также поддерживает операции, поддерживаемые **output** итератором. В таких итераторах из `it1 == it2` следует `it1++ == it2++`.
