...
Все итераторы поддерживают такие операции (итераторы, поддерживающие только такое, называются *forward*, и используются, к примеру, в std::forward_list и алгоритме partition):
...
но позволяют пройти по диапазону только один раз, то есть меняется состояние также всех копий итератора (например, поток ввода/вывода).
...
Самая мощная категория итераторов - *random access*. Ко всем предыдущим поддерживающимся в итераторах операциям добавляются:
- `it += n`
- `it -= n`
- `it + n`
- `n + it`
...
Примеры random access итераторов: std::vector, std::array, pointer. Random access итераторы требуются для функций, где необходимо уметь прыгать через n элементов: сортировки, k-я порядковая статистика, бинарный поиск и т.д.
