...
**Random access** итераторы свойственны таким контейнерам, как `std::vector`, `std::array`. Указатели также являются **random access** итераторами (на самом деле даже **contiguous**, описанными ниже). **Random access** итераторы требуются для функций, где необходимо уметь прыгать через n элементов: *сортировки, k-я порядковая статистика, бинарный поиск* и т.д.
