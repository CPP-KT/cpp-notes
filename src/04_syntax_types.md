...
Есть еще обычный `enum`, без слова `class`. Это тип-перечисление, который пришел из Си. Но им лучше не пользоваться, потому что:
 - у `enum` не прописано, какой внутри тип внутри (underlying type). Причём, не прописано именно в стандарте. То есть, это может отличаться от компилятора к компилятору.
 - они неявно конвертируются в `int`. Мы не хотим неявных конверсий.
 - и им еще можно не указывать спецификатор: можно просто `red` вместо `color::red`. Получается, они засоряют пространство имен. И, например, такой код не компилируется, потому что имена `c` конфликтуют (если заменить на `enum class`, то будет работать):
	```c++
...
А еще есть *функция `alignof`*.	Она возвращает *выравнивание* структуры. *Выравнивание* структуры - это число, на которое должен делиться тот адрес, по которому мы размещаем структуру. Использование: `alignof(mytype1)`, возвращает в нашем случае 4.
