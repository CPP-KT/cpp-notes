...
```
Указатели на функции подчиняются тем же правилам приоритета, что массивы и обычные указатели. При **декларатор указателя на функции считается суффиксным**.
...
На дробных числах promotion также есть (из `float` в `double`), но все операции с `float`'ами во `float`'ах и осуществляются. Если вы совершаете **операцию, один аргумент которой — целое число, а второй — числа с плавающей точкой, первое приводится ко второму**.
