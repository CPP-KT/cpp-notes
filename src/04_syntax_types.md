...
Чтобы понять, сколько занимают байт первая и вторая структура, узнаем, что такое:

1) **_sizeof_** - **оператор, который выдаёт кол - во байт**, выделенное для типа/структуры.
2) **_alignof_** - **оператор, который выдаёт выравнивание структуры.** Выравнивание структуры, это число на которое должен делиться текущий адрес, куда мы размещаем тип/структуру.

**alignof** - это всегда самый толстый тип в структуре. В нашем случае **`int`**.
А вот после в структуре все типы размещаются по этому **alignof**.
В случае **mytype1** мы видим **char - 1 байт**, а после идёт **int - 4 байта**. Наивные подумают, что размер структуры 5, но нет! Так как после char идёт адрес не выровненный по
alignof, то мы добавляем ещё 3 байта **padding**, а после вставляем **int**.

Итоговый **sizeof(mytype1) = 8 байт**. Аналогично для второй структуры.

https://cppinsights.io - сайт, где можно побаловаться с структурами.
### Объединение.
...
Поэтому, кстати, если вы хотите принимать в функцию все типы (чтобы они сохраняли численное значение), то вам хватит `int` и больше. Реализовать только `long long` и `unsigned long long` вы не можете, потому что `long`, например, не будет знать, куда ему конвертиться. А почему `char` и `short` будут? А потому что в языке есть 3 типа конверсий (*exact match*, *promotion* и *convertion*), каждый следующий хуже всех предыдущих, и если у вас есть два одинаково хороших варианта, то ошибка компиляции. Так вот конверсия из `short`'а в `int` — promotion, а `long` в `long long` или `unsigned long long` conversion'ы. Про всё это подробно можно почитать [тут](https://eel.is/c++draft/over.best.ics).
