...
Но если исключение всё-таки бросилось, то всё происходит достаточно медленно.
Причины:
- **Lock contention**
	**Unwinder** - раскручиватель стека. (мы у него можем спросить: вот если я сейчас сделаю `return`, то где я окажусь?). Когда летит исключение, надо понимать, откуда вызвана функция. Поэтому используется *unwinder*. *Unwinder* работает так: он смотрит специальные таблички, где написано, как раскручивать стек. И ему нужно смотреть в разные `.so` (`shared-object`) файлы (потому что функции, которые друг друга вызывают, могут быть в разных файлах). А эти файлы могут динамически загружаться-выгружаться. И разным потокам в многопоточной программе нужен доступ к списку загруженных `.so`. И они все обращаются к этому списку. В общем, проблема в том, что если у нас в нескольких потоках летят исключения, то это плохо параллелится и возникают зависимости между потоками.

- **Unwinder медленный**.
	Плюсовая поддержка исключений основана на языконезависимой поддержке (**Itanium ABI**). А у нее весь содержательный код в *unwinder*'е. Основное время программа проводит, раскручивая стэк. А сам обработчик исключений, когда поднимаемся на очередной уровень, просто каждый раз смотрит, не попал ли он в `catch` блок. И вот этот самый *unwinder*, которым раскручиваем стек, медленный.
	А почему не сделать его быстрым?
	Сложность разработки *unwinder*'а в том, что он должен почти не выделять память, потому что любое выделение может вызвать исключение.
	То есть он не может использовать почти никаких структур данных.
	А еще, информация про то, как расручивать стэк, которая хранится в файле, лежит не в таком виде, которым удобно пользоваться, а в таком, который занимает мало места.

- A еще **Itanium ABI** поддерживает такую фичу как resumable исключения.
	То есть у нас вылетело исключение, мы его поймали, а потом можем продолжить с той точки, откуда его выкинули.
	Но C++ такой фичи не имеет, но в реализации это всё равно есть.
	Это тоже лишняя сложность -- на самом деле стэк раскручивается дважды: один раз для того, чтобы найти catch, в который мы попали и узнать, resume'ящийся он или нет, а потом уже можно раскрутить как обычно.
