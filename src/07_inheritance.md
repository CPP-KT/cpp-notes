...
Так что тут у нас просто скопировались мемберы `base`'а, а мемберы `derived`'а потерялись.
...
```
С точки зрения языка, **вы не имеете права делать `delete` у базового класса, если создали наследуемый и не пометили деструктор базового как `virtual`**. Если будете так делать — UB. [**Даже если все деструкторы тривиальные.**](#множественное-наследование)
...
Если нам нужно всё из этого, то отнаследовать какую-либо фигуру от другой не получится. Но если у нас нет `set_*`, то методы, специфичные для прямоугольника, резко пропадают, а значит его можно отнаследовать от квадрата.
